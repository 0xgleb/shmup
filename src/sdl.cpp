...
Sdl::Sdl(const char *title, const char *path, const int width, const int height) {
	...
	this->_win =
		SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, 0);
	if(!this->_win) {
	...
	this->_renderer =
		SDL_CreateRenderer(this->_win, -1, SDL_RENDERER_ACCELERATED|SDL_RENDERER_PRESENTVSYNC);
...
void Sdl::render(Ship& player, List<Ship>& enemies) {
	...
	SDL_RenderCopy(this->_renderer, player.texture(), player.selection(), player.destination());
